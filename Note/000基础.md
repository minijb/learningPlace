...
**返回局部变量时不要画蛇添足**：程序会自动走快路（拷贝消除 / 优先移动），写了 `std::move(return_value)` 反而抑制它
...
简单来说：shared_ptr 只管「谁来销毁」；操作如何同步，它一概不管
...
简单来说：推荐用 copy-and-swap 完成值的转移——代价是**总会多一次拷贝**（进参时那一份）
